...
import { MouseEvent, useState } from "react"; // State management
...
export default function Header() {
  ...
  const [logoClickt, setLogoClickt] = useState<boolean>(false);

  const handleLogoClick = (e: MouseEvent<HTMLElement>) => {
    e.preventDefault();
    setLogoClickt(!logoClickt);
    setTimeout(() => router.push("https://ghislaine.network"), 550);
  };
      ...
      <div className={styles.header__logo}>
        <Link href="/" onClick={handleLogoClick}>
          <Image
            src="/logo.png"
            alt="Logo"
            className={logoClickt ? styles.clickclickclick : ""}
            width={58}
            height={58}
            priority
          />
